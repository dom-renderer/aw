        ...
        switch ($step) {
            ...
            case 3:
                ...
                $request->validate([
                    ...
                    '_tier_items.*.pricing_type' => 'required|in:0,1',
                    '_tier_items.*.min_qty' => 'nullable|numeric|min:1',
                ...
                foreach ($items as $index => $row) {
                    ...
                    $pricingType = (int)($row['pricing_type'] ?? 0);
                    
                    // Validate min_qty for tier pricing
                    if ($pricingType === 0 && (empty($row['min_qty']) || (float)($row['min_qty'] ?? 0) < 1)) {
                        return back()->withInput()->withErrors(["_tier_items.$index.min_qty" => 'Min quantity is required for tier pricing']);
                    }                    
                ...
                $tierItems = array_filter($items, function($row) {
                    return (int)($row['pricing_type'] ?? 0) === 0;
                });

                if (!empty($tierItems)) {
                    $grouped = [];
                    foreach ($tierItems as $row) {
                ...
                try {
                    ...
                    foreach ($items as $r) {
                        $pricingType = (int)($r['pricing_type'] ?? 0);
                        ProductTierPricing::create([
                            ...
                            'min_qty' => $pricingType === 1 ? 1 : (float)($r['min_qty'] ?? 1),
                            'max_qty' => $pricingType === 1 ? 0 : ($r['max_qty'] === null || $r['max_qty'] === '' ? 0 : (float)$r['max_qty']),
