...
class VariableProductController extends Controller
{
        ...
        switch ($step) {
            case 1:
                ...
                try {
                    ...
                    DB::commit();
                    return redirect()->route('product-management', ['type' => encrypt($request->type_switch), 'step' => encrypt(2), 'id' => encrypt($product->id)])
                    ...
                    switch ($request->op) {
                            ...
                            try {
                                ...
                                foreach ($request->input('attributes') as $attr) {
                                    ...
                                    foreach ($vals as $val) {
                                        ...
                                        $a = ProductAttribute::firstOrCreate(
                                            [
                                                'product_id' => $product->id,
                                                'title' => $title,
                                                'value' => $val,
                                ...
                                $processedVariantIds = [];
                                $items = [];
                                $counter = ProductVariant::where('product_id', $product->id)->max('id') ?? 0;
                                $counter++;
                                ...
                                foreach ($combinations as $set) {
                                    ...
                                    foreach ($existingVariants as $ev) {
                                        $evAttrIds = $ev->attributes->pluck('attribute_id')->sort()->values()->toArray();
                                        if ($evAttrIds === $set) {
                                            $existingVariant = $ev;
                                            break;
                                        }
                                    ...
                                    if ($existingVariant) {
                                        ...
                                        $parts = ProductAttribute::whereIn('id', $set)->pluck('value')->toArray();
                                        ...
                                        $variant = ProductVariant::create([
                                            ...
                                            'name' => $name,
                                            'sku' => $sku,
                                            'short_url' => strtolower(str_replace($name, ' ', '-')) . '-' . uniqid(),
                                            'barcode' => null,
                                            'status' => 1,
                                        ]);
                                    ...
                                    $parts = ProductAttribute::whereIn('id', $set)->pluck('value')->toArray();
                                    $img = ProductVariantImage::where('variant_id', $variant->id)
                                        ->where('is_primary', 1)
                                        ->first();
                                ...
                                if ($variantsToDelete->isNotEmpty()) {
                                    ProductAttributeVariant::where('product_id', $product->id)
                                ...
                                $usedAttributeIds = ProductAttributeVariant::where('product_id', $product->id)
                                    ->pluck('attribute_id')
                                    ->unique();
                        ...
                        case 'delete':
                            ...
                            try {
                                ...
                                ProductAttributeVariant::where('variant_id', $variant->id)->delete();
                        ...
                        case 'upload-image':
                            ...
                            $oldImage = ProductVariantImage::where('variant_id', $variant->id)
                                ->where('is_primary', 1)
                                ->first();
                        ...
                        case 'generate-barcodes':
                            ...
                            $baseCode = $product->id * 1000;

                            foreach ($variants as $i => $v) {
                                $v->barcode = sprintf('BC%06d', $baseCode + $i + 1);
                                $v->save();
                            }
                        ...
                        case 'list':
                            $variants = ProductVariant::where('product_id', $product->id)
                                ->with(['attributes.attribute', 'variantImage', 'product'])
                                ->get();
                            ...
                            $items = $variants->map(function ($v) {
                                return [
                                    ...
                                    'attributes' => $v->attributes->pluck('attribute.value')->toArray(),
                                    'image' => $v?->variantImage?->file ? asset('storage/' . $v->variantImage->file) : null,
                                ];
                            });
            ...
            case 4:
                ...
                $request->validate([
                    ...
                    '_non_tier_items.*.product_variant_id' => 'required|integer|exists:product_variants,id',
                ...
                foreach ($nonTierItems as $index => $row) {
                    $variant = ProductVariant::where('product_id', $product->id)->where('id', $row['product_variant_id'] ?? 0)->first();
                    if (!$variant) {
                        return back()->withInput()->withErrors(["_non_tier_items.$index.product_variant_id" => 'Invalid variant for this product']);
                    }
                ...
                try {
                    ...
                    foreach ($pricingTypes as $key => $pricingData) {
                        $variantId = $pricingData['variant_id'] ?? null;
                        $unitId = $pricingData['unit_id'] ?? null;
                    ...
                    \App\Models\ProductUnitPrice::where('product_id', $product->id)->delete();
                    ...
                    DB::rollBack();
                    Log::error('Failed to save pricing: ' . $th->getMessage());
                    return back()->withInput()->with('error', 'Failed to save pricing: ' . $th->getMessage());
                }
            ...
            case 5:
                ...
                $validated = $request->validate([
                    ...
                    'data.item_notes.*' => 'nullable|string|max:255'
                ]);
                ...
                if (isset($validated['data']['product_variant_id'])) {
                    foreach ($validated['data']['product_variant_id'] as $index => $product_variant_id) {
                        ...
                        if ($inventory) {
                            $existingStock = $inventory->quantity;
                            $inventory->update([
                        ...
                        $historyQ = $isNew ? 1 : (
                            $existingStock - $item_quantity > 0 ? $existingStock - $item_quantity : (
                                $existingStock - $item_quantity == 0 ? 0 : $item_quantity - $existingStock
                            )
                        );

                        $historyT = $isNew ? 1 : (
                            $existingStock - $item_quantity > 0 ? 0 : (
                                $existingStock - $item_quantity == 0 ? 1 : 1
                            )
                        );
    ...
    public function getVariantStockHistory(Request $request) {

        if ($request->method() == 'GET') {

            if ($request->pt == 'simple') {
                $product = $request->filter_product;
                $warehouse = $request->filter_warehouse;

                $histories = InventoryHistory::where('product_id', $product)
                ->where('warehouse_id', $warehouse)
                ->latest();
            } else {
                $variant = $request->filter_variant;
                $warehouse = $request->filter_warehouse;

                $histories = InventoryHistory::where('product_variant_id', $variant)
                ->where('warehouse_id', $warehouse)
                ->latest();
            }
            ...
            ->editColumn('type', function ($row) {
                return $row->type == 0 ? 'Out' : 'In';
            })
            ->editColumn('quantity', function ($row) {
                return $row->type == 1 ? '<span class="text-success">+' . $row->quantity . '</span>' : '<span class="text-danger">-' . $row->quantity . '</span>';
            })
            ->addIndexColumn()
            ->rawColumns(['type', 'quantity'])
            ->toJson();
        }
    }
    ...
    public function adjustStock(Request $request) {
        if ($request->isMethod('get')) {
            if ($request->pt == 'simple') {
                ...
                $inventory = Inventory::where('product_variant_id', $request->variant_id)
                    ->where('warehouse_id', $request->warehouse_id)
                    ->first();
